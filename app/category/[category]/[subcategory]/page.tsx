...
export const generateMetadata = ({params}:{params:{subcategory:string}}) =>{
    return{
        title:`AnimeEcom - ${params.subcategory}`,
        description:`Discover ${params.subcategory} items`
    }
}
...
const page =async ({params,searchParams}:{params:{subcategory:string,category:string},searchParams:{ [key: string]: string | string[] | undefined }}) => {
    ...
    return (
        ...
        <SearchedContent sort={sortParams} searchQuery={searchURL} cursor={cursor} products={products} searchValue={`${params.category} > ${params.subcategory}`}/>
        </div>
    )
}
